/* SUPER GOLD MODULE v2.0 - integrated feature. Software integration only;
   the trading/strategy logic, scoring, risk guidance, and broker order handoff
   are user-owned and NOT evaluated or endorsed. Manual verification required.
...
/* Always-on Super Gold main evaluation. Integration only - not endorsed. */
async function runSuperGold(){
  ...
  try{
    ...
    var swingPlanHtml="<div class=\"plan\">"+planBlock(casc,entry,stop,t1,t2||t1)+"</div>"
      +"<button class=\"toTrade\" onclick=\"toTrade('XAUUSD','"+dirForTrade+"',"+entry+","+stop+","+t1+")\">SEND TO TRADE PLAN (XM 360)</button>"
    ...
    function sgScalpLedger(dir){
      ...
      if(swept&&reclaimed){
        ...
        g.push(["C15","2R to opposite pool",scalpRr>=2?"pass":"veto",isFinite(room2)?"room "+px(room2)+" 2R "+px(2*scalpRisk):"-"]);
      }else{
        scalpStop=dir==="long"?lastClose-1.5*a15:lastClose+1.5*a15;
        scalpRisk=Math.abs(scalpEntry-scalpStop);
        if(scalpRisk>0){ scalpT1=dir==="long"?scalpEntry+2*scalpRisk:scalpEntry-2*scalpRisk; scalpT2=dir==="long"?scalpEntry+3*scalpRisk:scalpEntry-3*scalpRisk; }
        g.push(["C15","2R to opposite pool","na","no sweep - using ATR structure. Watch for sweep."]);
      }
      ...
      if(scalpRisk>0){
        ...
        planHtml="<div class=\"plan\">"+planBlock(dir,scalpEntry,scalpStop,scalpT1,scalpT2||scalpT1)+"</div>"
          +"<button class=\"toTrade\" onclick=\"toTrade('XAUUSD','"+dir+"',"+scalpEntry+","+scalpStop+","+scalpT1+")\">SEND TO TRADE PLAN (XM 360)</button>"
...
function updateSgDxy(){ var sel=$("sgDxySelect"); if(sel&&$("sgDxy")) $("sgDxy").innerHTML="DXY <b>"+sel.value.toUpperCase()+"</b>"; }
function updateSgTnx(){ var sel=$("sgTnxSelect"); if(sel&&$("sgTnx")) $("sgTnx").innerHTML="TNX <b>"+sel.value.toUpperCase()+"</b>"; }

window.runSuperGold=runSuperGold;
window.updateSgDxy=updateSgDxy;
window.updateSgTnx=updateSgTnx;
})();
